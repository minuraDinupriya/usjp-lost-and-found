...
app.post('/api/items', verifyToken, upload.single('image'), async (req, res) => {
  ...
  } catch (error) { res.status(400).json({ message: 'Error saving item' }); }
});
...
app.delete('/api/items/:id', verifyToken, async (req, res) => {
  try {
    ...
    if (item.createdBy !== req.userId) return res.status(403).json({ message: 'Only owner can delete' });
...
app.put('/api/items/:id', verifyToken, upload.single('image'), async (req, res) => {
  try {
    const item = await Item.findById(req.params.id);
    if (item.createdBy !== req.userId) return res.status(403).json({ message: 'Only owner can edit' });
    const updateData = { ...req.body };
    if (req.file) updateData.imageUrl = `http://localhost:${PORT}/uploads/${req.file.filename}`;
...
app.patch('/api/items/:id/claim', verifyToken, async (req, res) => {
  try {
    const item = await Item.findById(req.params.id);
    if (!item) return res.status(404).json({ message: 'Item not found' });
    if (item.createdBy !== req.userId) return res.status(403).json({ message: 'Only owner can mark solved' });
    item.status = 'Claimed';
    await item.save();
    res.json(item);
